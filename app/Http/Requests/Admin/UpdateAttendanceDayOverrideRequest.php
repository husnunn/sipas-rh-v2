...
class UpdateAttendanceDayOverrideRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'event_type' => ['sometimes', 'required', 'in:early_dismissal,teacher_meeting,special_event,holiday_override,attendance_closed,custom'],
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                ...
                $existing = $this->route('attendanceDayOverride') ?? $this->route('attendance_day_override');
                $data = array_merge(
                    $existing?->only([
                        'check_in_open_at',
